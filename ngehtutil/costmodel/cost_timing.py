...
def calculate_costs_over_time(cost_config, array_name, array_costs):
    ...
    for year in range(calculation_start, final_year+1):
        ...
        costs_over_time.loc[year,f'{array_name} - CAPEX'] = float(cap_spend)
        costs_over_time.loc[year,f'{array_name} - OPEX'] = float(op_spend)
